...
const DEBUG_OBJECT_IDS = /* @__PURE__ */ new WeakMap();
...
function typeForRpc(value) {
  ...
  switch (prototype) {
    ...
    default:
      ...
      if (value instanceof RpcTarget$1) {
        return "rpc-target";
      }
...
let mapImpl = { applyMap: mapNotLoaded, sendMap: mapNotLoaded };
...
class ErrorStubHook extends StubHook {
  constructor(error) {
    super();
    this.error = error;
  }
...
let RAW_STUB = /* @__PURE__ */ Symbol("realStub");
...
class RpcStub extends RpcTarget$1 {
...
class RpcPromise extends RpcStub {
...
class ValueStubHook extends StubHook {
  call(path, args) {
    try {
...
class PayloadStubHook extends ValueStubHook {
  constructor(payload) {
    super();
...
let __experimental_newHibernatableWebSocketRpcSession = __experimental_newHibernatableWebSocketRpcSession$1;
class DurableCounterProxy extends RpcTarget {
  ...
  async increment(amount = 1) {
    const current = await this.ctx.storage.get(`counter:${this.key}`) ?? 0;
...
class HibRpcDo extends DurableObject {
  ...
  async webSocketClose(ws, code, reason, wasClean) {
    ...
    const session = sid ? this.sessions.get(sid) : void 0;
    session?.handleClose(code, reason, wasClean);
    ...
    const session = await __experimental_newHibernatableWebSocketRpcSession(
      ...
      new RootTarget(this.ctx, this.env, this),
      {
        ...
        __experimental_trace: (event) => {
          ...
          if (event.phase === "receive" || event.phase === "readLoop.push" || event.phase === "readLoop.pull" || event.phase === "readLoop.resolve" || event.phase === "readLoop.reject" || event.phase === "send" || event.phase === "ensureResolvingExport.start" || event.phase === "ensureResolvingExport.resolve" || event.phase === "ensureResolvingExport.reject" || event.phase === "getOrRestoreExportHook.restore") {
...
const index = {
  async fetch(request, env) {
    ...
    if (url.pathname === "/ws" || url.pathname === "/instance-id" || url.pathname === "/attachments" || url.pathname === "/resume-diagnostics") {
