...
var CborCodec = class {
  ...
  constructor(options = {}) {
    ...
    const useRecords = CAN_GENERATE_CODE && !globalThis[WORKERS_MODULE_SYMBOL];
  ...
  __experimental_reset() {
    ...
    this.encoder = new Encoder({
      ...
      useRecords: CAN_GENERATE_CODE && !globalThis[WORKERS_MODULE_SYMBOL],
      encodeUndefinedAsNil: false,
      tagUint8Array: true,
...
var RpcSessionImpl = class {
  ...
  __experimental_snapshot() {
    ...
    for (let i in this.imports) {
      if (+i !== 0) {
        throw new Error(
          "Can't snapshot RPC session with peer-imported capabilities beyond the bootstrap import."
        );
      }
    }
    const exports$1 = [];
    for (let i in this.exports) {
      ...
      if (!entry) continue;
      if (entry.pull) {
        throw new Error("Can't snapshot RPC session with outstanding promise exports.");
      }
      ...
      if (!descriptor) {
        throw new Error(
          `Export ${id} is not hibernatable. Only registry-backed RpcTargets can survive restore.`
        );
      }
      exports$1.push({ id, refcount: entry.refcount, descriptor });
    }
    return {
      version: 1,
...
async function __experimental_newHibernatableWebSocketRpcSession(webSocket, localMain, options) {
  ...
  if (!sessionId) {
    ...
    webSocket.serializeAttachment?.({ sessionId, version: 1 });
  }
...
function getAttachedSessionId(webSocket) {
...
export { CborCodec, RpcHandlerRegistry, RpcPromise2 as RpcPromise, RpcSession2 as RpcSession, RpcStub2 as RpcStub, RpcTarget4 as RpcTarget, __experimental_newDurableObjectSessionStore, __experimental_newHibernatableWebSocketRpcSession2 as __experimental_newHibernatableWebSocketRpcSession, __experimental_resumeHibernatableWebSocketRpcSession2 as __experimental_resumeHibernatableWebSocketRpcSession, cborCodec, newHttpBatchRpcResponse, newHttpBatchRpcSession2 as newHttpBatchRpcSession, newMessagePortRpcSession2 as newMessagePortRpcSession, newWebSocketRpcSession2 as newWebSocketRpcSession, newWorkersRpcResponse, newWorkersWebSocketRpcResponse, nodeHttpBatchRpcResponse };
